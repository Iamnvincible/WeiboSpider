...
from content_parser.user_info.public import get_left, get_right
...
def get_detail(html, uid):
    ...
    for each_module in basic_modules:
        try:
            basic_str = each_module.find(
                attrs={'class': 'main_title W_fb W_f14'}).get_text()
            if '基本信息' in basic_str:
                for each in basic_info:
                    each_str = each.get_text()
                    if '昵称：' in each_str:
                        user.name = each.find(
                            attrs={'class': 'pt_detail'}).get_text()
                    elif '所在地：' in each_str:
                        user.location = each.find(
                            attrs={'class': 'pt_detail'}).get_text()
                    elif '性别：' in each_str:
                        gender = each.find(
                            attrs={'class': 'pt_detail'}).get_text()
                        ...
                        user.birthday = each.find(
                            attrs={'class': 'pt_detail'}).get_text()
                    elif '简介：' in each_str:
                        description = each.find(
                            attrs={'class': 'pt_detail'}).get_text()
                        user.description = description.encode(
            ...
            if '标签信息' in basic_str:
                basic_info = each_module.find_all(
                    attrs={'class': 'li_1 clearfix'})
                for each in basic_info:
            ...
            if '联系信息' in basic_str:
                ...
                for each in basic_info:
                    ...
                    if '邮箱：' in each.get_text():
                        contact_info.append(
                            'email:' + each.find(attrs={'class': 'pt_detail'}).get_text())
                    if 'MSN：' in each.get_text():
                        contact_info.append(
                            'msn:' + each.find(attrs={'class': 'pt_detail'}).get_text())
